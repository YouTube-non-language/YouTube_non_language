...
class poseDetector() :
    ...
    def findPose_webcam(self, img, draw = True) :
        """
        Input
            1) img :
                이미지
        ...
        Output
            1) img :
                ...
                draw 여부에 따라 랜드마크 점 표시 및 점을 선으로 연결
        """
        # cv2의 BGR을 mediapipe가 인식 가능하도록 RGB로 변환 & 이미지를 좌우를 반전
...
def main(video) :
    ...
    while True :
        ...
        # 강조하고 싶은 포인트
        face_point_1 = 0 # 0 - nose / 코
        face_point_2 = 2 # 2 - left eye / 왼쪽 눈
        face_point_3 = 5 # 5 - right eye / 오른쪽 눈
        face_point_4 = 9 # 9 - mouth (left) / 입 (왼쪽)
        face_point_5 = 10 # 10 - mouth (right) / 입 (오른쪽)
        ...
        if len(lmList) != 0 :
            print(lmList[face_point_1], lmList[face_point_2], lmList[face_point_3], lmList[face_point_4], lmList[face_point_5],
                lmList[body_point_1], lmList[body_point_2], lmList[body_point_3], lmList[body_point_4], lmList[body_point_5], lmList[body_point_6])
            
            cv2.circle(img, (tuple(lmList[face_point_1][1:])), 5, (0, 255, 0), cv2.FILLED)
            cv2.circle(img, (tuple(lmList[face_point_2][1:])), 5, (0, 255, 0), cv2.FILLED)
            cv2.circle(img, (tuple(lmList[face_point_3][1:])), 5, (0, 255, 0), cv2.FILLED)
            cv2.circle(img, (tuple(lmList[face_point_4][1:])), 5, (0, 255, 0), cv2.FILLED)
            cv2.circle(img, (tuple(lmList[face_point_5][1:])), 5, (0, 255, 0), cv2.FILLED)
            
            cv2.circle(img, (tuple(lmList[body_point_1][1:])), 5, (255, 0, 0), cv2.FILLED)
            cv2.circle(img, (tuple(lmList[body_point_2][1:])), 5, (255, 0, 0), cv2.FILLED)
            cv2.circle(img, (tuple(lmList[body_point_3][1:])), 5, (255, 0, 0), cv2.FILLED)
            cv2.circle(img, (tuple(lmList[body_point_4][1:])), 5, (255, 0, 0), cv2.FILLED)
            cv2.circle(img, (tuple(lmList[body_point_5][1:])), 5, (255, 0, 0), cv2.FILLED)
            cv2.circle(img, (tuple(lmList[body_point_6][1:])), 5, (255, 0, 0), cv2.FILLED)
